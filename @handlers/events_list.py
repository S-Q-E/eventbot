...
async def list_events_by_category(callback: types.CallbackQuery):
    ...
    try:
        ...
        logging.error(f"Invalid callback data format for filter_cat: {callback.data}")
    ...
    logging.info(f"Received callback for category filter: cat_id={cat_id}, page={page}")
    ...
    with get_db() as db:
        ...
        if not category:
            logging.error(f"Category with ID {cat_id} not found for user {callback.from_user.id}.")
            await callback.answer("Ошибка: Выбранная категория не найдена в базе данных.", show_alert=True)
            return
        
        logging.info(f"Processing category: '{category.name}' (ID: {cat_id})")

        # Query for all events in the category, without date filter, for debugging
        all_events_in_category = db.query(Event).filter(Event.category_id == cat_id).all()
        logging.info(f"Found {len(all_events_in_category)} total events for category '{category.name}' (ID: {cat_id}).")

        # Query for future events in the category
        events = (
            db.query(Event)
        ...
        logging.info(f"Found {len(events)} future events for category '{category.name}' (ID: {cat_id}).")
        ...
        if not events:
            ...
            # Consider editing the message if it's the one showing category buttons,
            # or sending a new message if events are expected in separate messages.
            # The current code sends events in separate messages, so this edit might be problematic.
            # Let's assume the original message was the one with category buttons.
            try:
                await callback.message.edit_text("События в этой категории отсутствуют.", reply_markup=keyboard.as_markup())
            except TelegramAPIError: # If message is too old or already edited, send new one
                 await callback.message.answer("События в этой категории отсутствуют.", reply_markup=keyboard.as_markup())
            return
        ...
        for event in slice_events:
            ...
            # The original code sends events one by one.
            # This might lead to a very long message history if there are many events.
            # For debugging, this is fine. If there's an issue with sending these, it might be a separate problem.
